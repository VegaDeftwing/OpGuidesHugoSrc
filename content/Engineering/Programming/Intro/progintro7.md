...
Problem is, somebody could do somethnig like
...
which would crash the program, because we've exceded the length limit we defined above, so, maybe we make a function to set the first name:
...
And of course, there'd be yet more code to actually let a player use the item once it's in their inventory. What you should note though, is that when calling `PickTool(1,"Wooden Pick",1,1)` a new object of that type is made. If another one is crafted, it will still be a new item, with it's own durability, name, etc. Even if they're the same, if one is used it won't use the other.
...
There's two dogs being "made". If both were named "Spot" that would be fine, but it's still two different dogs. Just like how meeting someone with your same name doesn't make the two of you the same person.
...
* **O** - Open-Closed Principal
  ...
  * In other words, you should be able to change how something works, as much as reasonable, by extending a class without changing it's source code.
...
* **D** - Dependency Inversion Principal
  ...
  * This does make sense if you're making something in a VERY big project, working with mulitple teams, etc.
