...
Text to image is great, but image-to-image is scary powerful too. If you're going though pictures and you find something close to what you want, you can pluck it out and work with variations of it.
...
{{< smalltext >}}The prior image, though img2img{{< /smalltext >}}

<--->

![grid-0048](/nonfree/blog/ai/grid-0048.webp)

{{< smalltext >}}The prior image with the background, though img2img again.{{< /smalltext >}}

{{< /columns >}}

### In painting

When photo editing sounds to tedious, just erase and generate.
...
This might be different if you have an extremely generic character "Red head with big muscles" or "A white wolf with red eyes" but otherwise, no way.
...
Getting a specific number of anything can be difficult, especially body parts (eyes, legs, etc.). This otter should have 3 eyes, but convincing it to actually add those eyes is tough.
...
Getting extra limbs is a pretty common occurance, as is doubled up torsos, weird eyes and mouths, and unsettling numbers of fingers. This example is actually on the pretty tame side. You'll often get some really, really creepy things out.
...
**Stable Diffusion**? Well, that's free-ish. Problem is, you'll probably want to run it locally, which requires a really, really beefy graphics card. I was struggling to run it on a Vega56 - a GPU that goes for ~$150 used now - so went out and got a RTX3090 for about \$1,000. If you're already a gamer with a GPU with 8Gb+ of VRAM you're probably good, but for most people this is a bit absurd. It's even worse If you want to train your own models to make specific-purpose models ([such as these, most of which are NSFW](https://rentry.org/sdmodels)).

## The photography analogy

I don't know who said it first, but this entire problem isn't a new one. This has all happened before, when artist got up in arms because the camera was coming for their jobs as portrait artists. And you know what? They were right. It cost them money, and lowered the need for painters. As a society, we gained the ability to capture moments instantly. Eventually this turned into recordings (and something in between, like "Live Photos") and the idea of hiring a painter to come to events to get a picture sounds incredibly foreign. If the goal is to capture life as it is, the camera is clearly more accurate than any painter. 

The camera also spawned industry, as operating one well - while it is getting easier - is still an art in it's own right and so photography is a business.
...
But, to wrap back around to the larger point: did the invention of the camera entirely end the desire for painted artwork? Of course not. It dramatically lowered demand for portraits, but over time the lower cost of photography also meant more money on hand for art works with actual creative intent instead of those simply aiming to caputure still headshots. I don't think there's many artists who'd want to paint headshots in the same style every day for a living.

AI art models may have this same effect, picking some of the low hanging fruit so that artists must climb up the tree of originally and expression to justify their works. This is related to the barrier to entry points above, but I think finally lets me state the biggest takeaway from this entire post:

AI art will replace demand for artwork that artists made in bulk anyway. Nor will it prevent those with passion for making art from making it.
...
If you're already doing the work for less than minimum wage, does it being free mater? I'm asking that as a real question, not rhetorical. Do you care if the people that already weren't willing to pay you above minimum wage now have a tool which will generate something of a lesser-but-near quality to use instead? Maybe you do, maybe you make art in a way where you are making above minimum wage either by high prices or by doing in incredibly fast. I'm not trying to give you an opinion, I'm trying to help you ask the questions to make an informed one.
...
It also serves to allow projects that would otherwise be blocked on the need for artists and so never exist to get started. That is, by *not paying for art* different ideas can be lower risk to try. For example,
...
## It's far from "replacing" yet anyway.
...
it's a tool that use other people works without permission! AI pictures have even tried copying watermarks before!

{{< /speech >}}

Due to the way the model works, just because a watermark is present, doesn't mean it's at all related to the art generated. The way the model works it doesn't draw inspiration from one source or draw inspiration from specific pieces. If a recognizable watermark appears, it's because the model has found that pictures with matermarks in general tend to look better/match your prompt and so generates one. There *can* be relationships - if you ask for a generic corporate art picture you might see a Shutterstock logo - but if there's a large pool of artists making art that is valid for the prompt then the particular watermark is unrelated. It's effectively random chance.
...
Clearly, the same thing still applies in AI. If someone makes a fine tuned model without an artists permission, that's pretty fucked. But it's not like that problem didn't exist before, this just - you guessed it - lowers the barrier to entry for doing so. It also allows for it do be done en mass, which is pretty bad.

## "I'd rather look at art made by a human"

{{< columns >}}

This is a really hard argument to even talk about because it lacks a "why" that we can put in any scientific sense. If art is really good any we find out that - while it does require some human input - it is done via AI, we feel created or we feel that the creator cheated.
...
Similarly, there's now quite a few Pron subreddits that are entirely focused on AI generated imagry, for example, [r/MonsterGirlAI (NSFW, obviously)](https://www.reddit.com/r/MonsterGirlAI/). We're letting computers make something that we find sexually interesting. Sure, if you're in the act you can not think about it, but after words? You have to come to grips with having just gripped to come when looking at pictures that a human didn't even draw. There is something about that which sets of alarm bells in the same uncanny valley section of my brain as some humanoid robots do while the images in that subreddit themself don't. It's uncomfortable.

Ultimately, we're implying art "generated" by a human is better than art generated by a machine for some intrinsic quality that we can not measure. You could claim it to be scarcity, but in what sense? We've already established there's more art than anybody could view in a life time being produced at a rate exceeding our ability to view it by humans. What does adding AI matter if that threshold has been crossed?
...
This, I think (though don't know) is the true reason so many people are uncomfortable with AI art. It's good enough to pass. Often, you can tell if an image is AI generated visually, and if by nothing else, the embedded metadata added with most existing tools. But, sometimes, you can't. That means we can feel deceived into thinking someone put more effort into their work than they truly did, and we as a society aren't used to valuing art not made by man. This makes us be skeptical of the value of any artwork we see, less it be made my a machine.

I suspect this is something that those of us that have been brought up in a culture without this will never know how to respond to or feel about, and the coming generations will find way to use to do things we can't imagine.
